...
eval "$(grep -E '^(API_URL|PUBLISHABLE_KEY|SECRET_KEY|DB_URL)=' <<<"$env_lines")"
export SUPABASE_URL="$API_URL" SUPABASE_PUBLISHABLE_KEY="$PUBLISHABLE_KEY" SUPABASE_SECRET_KEY="$SECRET_KEY" SUPABASE_DB_URL="$DB_URL"
exec "$@"
