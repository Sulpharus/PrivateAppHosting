...
function layout(
  ...
  return `<!doctype html>
<html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(heading)}</title></head>
...
<h1 style="margin:0 0 12px;font-size:24px;line-height:1.2">${escape(heading)}</h1>
<p style="margin:0 0 24px;font-size:16px;line-height:1.5;color:#3d3a33">${body}</p>
<a href="${escape(action.url)}" style="display:inline-block;padding:14px 22px;border-radius:12px;background:#b8431a;color:#ffffff;font-weight:600;font-size:16px;text-decoration:none">${escape(action.label)}</a>
<p style="margin:24px 0 0;font-size:13px;line-height:1.5;color:#5f5a50">${escape(footer)}</p>
...
export function inviteEmail(input: { inviterName: string; url: string; expiresAt: Date }): Email {
  ...
  const body = `${escape(input.inviterName)} hat dir Zugang zu MiniNode eingerichtet. Lege mit einem Klick deinen Account an – am einfachsten mit Fingerabdruck oder Gesichtserkennung.`;
  return {
...
export function authEmail(action: AuthEmailAction, input: { url: string; code?: string }): Email {
  ...
  const codeLine = input.code
    ? ` Dein Code: <strong style="font-family:monospace;font-size:18px">${escape(input.code)}</strong>`
    : '';
  return {
    subject: copy.subject,
    html: layout(
      copy.heading,
      `${escape(copy.body)}${codeLine}`,
